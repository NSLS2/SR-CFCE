...
//----------------------------------------------------------------------------
    ...
    AERR(createParam("CNT T", asynParamFloat64Array, &P_cnt_t));
...
/* September 25, 2020					*/
    ...
    AERR(createParam("NA I",   asynParamInt16Array, &P_na_I));
/*							*/
/********************************************************/
/********************************************************/
/*							*/
...
/********************************************************/
    ...
    AERR(createParam("DAC Time", asynParamFloat64Array, &P_ff_T));
...
/*------------------------------------------------------*/
    ...
    AERR(createParam("FF IQ", asynParamFloat64Array, &P_ff_IQ));
    AERR(createParam("FB IQ", asynParamFloat64Array, &P_fb_IQ));

    AERR(createParam("Ref Phase", asynParamFloat64, &P_ref_phase));
    AERR(createParam("Scope T", asynParamFloat64, &P_adc_T));

    AERR(createParam("ADC I", asynParamFloat64Array, &P_adc_I));
    AERR(createParam("ADC Q", asynParamFloat64Array, &P_adc_Q));
...
try {
    ...
    } else if(function==P_arcDetEnbl) {

        int    ep  = 0;
        int    val = 0;
        int    mask= ARCDET_ENABLE;

        lock();

	val = value ? ARCDET_ENABLE : 0;

        HWERR2(pasynUser, okFrontPanel_SetWireInValue(H, ep, val, mask));
	okFrontPanel_UpdateWireIns(H);

	printf("P_arcDetEnbl: ep=0x%04x, value=0x%04x, mask=0x%04x\n", ep, val, mask);

        asynPrint(pasynUser, ASYN_TRACEIO_DRIVER, "%s: ARCDET Enable:\n",
	                  portName);
        unlock();
...
try {
    ...
    // printf("llrfPort.cpp: writeFloat64Array\n");
...
try {
    ...
    // printf("llrfPort.cpp: readFloat64Array\n");
